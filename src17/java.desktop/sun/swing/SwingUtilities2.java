...
public class SwingUtilities2 {
    ...
    // tsbMobile: FlatLafs Java-8-Pfad. FlatLaf entscheidet nach java.version, ob es die
    // Kantenglaettung ueber putAATextInfo (Java 9+) oder ueber Reflection auf dieses Feld und
    // diese Klasse (Java 8) eintraegt. Android meldet java.version "0" — unveraenderlich, die
    // Eigenschaft gehoert zu Androids festen Systemwerten — also nimmt FlatLaf dort den alten
    // Weg und scheiterte mit NoSuchFieldException: AA_TEXT_PROPERTY_KEY, worauf es auf Metal
    // zurueckfiel (gemessen auf Android 14). Beides gab es bis JDK 8; hier steht es wieder,
    // so weit FlatLaf es braucht. Der Wert landet in den UIDefaults und wird von nichts
    // anderem gelesen.
    ...
    public static class AATextInfo {
        ...
        public static AATextInfo getAATextInfo(boolean lafCondition) {
            return new AATextInfo(java.awt.RenderingHints.VALUE_TEXT_ANTIALIAS_ON, null);
        }
    }
    ...
    public static int getLeftSideBearing(JComponent c, FontMetrics fm,
                                         char firstChar) {
        // tsbMobile: hier stand eine Zwischenablage, die ueber Font.createGlyphVector die
        // linke Seitenlage des ersten Zeichens ermittelt — die Strecke, um die ein "W"
        // optisch weiter links beginnt als sein Kaestchen. Swing rueckt Beschriftungen
        // darum ein paar Pixel zurecht.
        //
        // Der Weg dorthin fuehrt ueber StandardGlyphVector, FontUtilities.getFont2D und
        // FontManagerFactory — also in sun.font hinein, und dort steht auf dem Telefon
        // nichts. Null heisst: die Beschriftung sitzt an ihrem Kaestchen statt an ihrer
        // Kontur. Auf einem Telefon ist das unsichtbar. (Wie im JDK-8-Port.)
        return 0;
    }
    ...
    public static FontMetrics getFontMetrics(JComponent c, Font font) {
        // tsbMobile: hier stand 'return FontDesignMetrics.getMetrics(font, frc)' — der
        // Eingang zu sun.font: FontDesignMetrics → SunFontManager → StrikeCache → FreeType.
        // Auf dem Telefon misst die Plattform (Skia, CoreText); tsb.port.Schrift fragt sie.
        // Der FontRenderContext faellt damit weg. (Wie im JDK-8-Port.)
        return tsb.port.Schrift.metrik(font);
    }
